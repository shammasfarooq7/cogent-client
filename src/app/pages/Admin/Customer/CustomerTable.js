...
import { GET_ALL_CUSTOMERS_QUERY } from '../../../../graphql/admin';
import { CustomerView } from './CustomerView';


export const CustomerTable = ({ tableName, search, setTicketTabelRefetch, ticketTableRefetch, todays=false}) => {
    ...
    const { data, loading, refetch } = useQuery(GET_ALL_CUSTOMERS_QUERY, {
        variables: {
            getAllCustomerInput: {
                page,
                limit,
    ...
    const handleViewClick = (id) => {
        const currentTicket = data?.getAllCustomer?.customers.filter((ticket) => ticket.id === id)
    ...
    return (
        <>
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
                <Typography sx={{ color: "black", fontWeight: "600", fontSize: "18px" }}>Customer's Listing</Typography>
                <Box>
                ...
                <TableBody sx={{ marginTop: "10px" }}>
                    ...
                    {loading
                        ...
                        : data?.count && data.count == 0
                            ...
                            : data?.getAllCustomer?.customers.map((ticket) => (
